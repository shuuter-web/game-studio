...
`<script>` 内のJSを **BOMなしUTF-8** で抽出して `node --check`。`Out-File` はUTF-16になるので使わない。

```powershell
$path = "C:\Users\syuta\OneDrive\game-studio\prototypes\dragon-tide\index.html"
$c = [System.IO.File]::ReadAllText($path)
$m = [regex]::Match($c, '(?s)<script>(.*)</script>')
$out = "$env:TEMP\dt_check.js"
[System.IO.File]::WriteAllText($out, $m.Groups[1].Value, (New-Object System.Text.UTF8Encoding($false)))
node --check $out
"EXITCODE: $LASTEXITCODE"
```

EXITCODE: 0 以外なら修正してから先に進む。
