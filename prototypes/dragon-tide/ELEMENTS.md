...
> **入口は [dragon-tide-overview.md](../../docs/gdd/dragon-tide-overview.md)（総括・実装 v0.99.3 準拠）。**
...
## 2. 動く敵（mover）— `MOVER_DEFS`（index.html:2249）

マップを巡回してくる移動する敵。建物ではない。時代ごとに顔ぶれが変わる。
**数える単位**: 個体は「体」、湧きの枠（`MOVER_SPAWNS_BY_STAGE` の1エントリ）は「部隊」。
`packCount` を持つ敵は1部隊が複数体で湧く（マンモス3体・騎士8体など）。
