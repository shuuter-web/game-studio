...
**v0.37 で「極端な動き」用のチャネルを4つ足した**（既定はすべて無効＝従来挙動）。
プリセット名には畳まず、竜種ごとに自由に組み合わせられる形を保っている。

| 項目 | 既定 | 意味 |
|---|---|---|
| `orbitRadius` | 0 | >0 で、操縦点にこの距離まで寄ったら**接線方向へ回り込む**。等速で止まれない竜が操縦点の上で往復・振動するのを防ぐ（§12-2d） |
| `hopPeriod` | 0 | >0 で**跳躍移動**（蛙型）。1サイクルの秒数 |
| `hopFlightFrac` | 0.42 | サイクルのうち滞空している割合。滞空中は**旋回不能（弾道）**、残りは着地して静止 |
| `hopGroundFrac` | 0.06 | 着地中の速さ（`maxSpeed` 比）。0にすると速度ベクトルが消えて**向きを見失う**ので微速で残す |
| `hopPathSkip` | 0 | >0 でリーダーのパス先読みをこの点数にする＝**1回の跳躍でパスを飛ばして処理**する |
| `pathTrace` | false | true で**描いた線を弧長でそのままなぞる**（角を丸めない）。群れも線上の隊列スロットを追う（§12-2e） |
| `controlScheme` | "path" | 描いた線の**解釈**。"path"=軌跡として追う（従来）／"vector"=**向きの指定**として読む（§12-2f） |

- 滞空中の速さは `1 / hopFlightFrac` 倍になるので、**平均速度は `maxSpeed` に一致する**
  （実測：蛙竜 hopPeriod 0.7 で 9 ↔ 357px/s を往復し、平均148 ≒ maxSpeed 150）。
- 跳躍中の `turnRate` は「**着地して向きを合わせるときの旋回速度**」の意味になる
  （滞空中は 0 に固定＝実測の滞空中回転量 0.00000rad）。
- 描画は `boidHopAir[i]`（0=着地／1=頂点）を simStep が書き、drawBoids が
  **サイズ（+35%）と真下の影**で高さを表す（トップビューなので大きさでしか高さを出せない）。
  実測：頂点で描画面積 2.09倍。

攻撃側（`ATTACK_DEFS`）が持つもの：`kind`（攻撃処理の分岐キー）／`recoilDist`（反動）／
`fireStun`（硬直 秒）。取得は **`atk()`**。

### 12-2d. 回り込み（`orbitRadius`）— v0.37

**等速（`minSpeed === maxSpeed`）の竜は操縦点の上で止まれない。** そのまま突っ込ませると
通り過ぎ→反転→通り過ぎ…の往復になるので、`orbitRadius` の内側では操縦方向を
**接線＋半径のズレ補正**に差し替えて周回に落とす。ここで踏んだ罠が2つある。

1. **回転方向を個体ごとに決めると半数が逆回りになって正面衝突する。**
   実測：平均0.30rad/フレームの方向転換・符号反転11.4回/秒＝激しい震え。
   → **群れの角運動量の符号で全員の向きを揃える**（`_orbitSide`。しきい値
   `ORBIT_SIDE_HYSTERESIS = 4000` で0付近のパタつきを止める）。
2. **即時方向転換の竜は、分離力を「向き」に混ぜた瞬間に震える。**
   隣とのわずかな押し合いがそのまま毎フレームの方向転換になる（40体で0.15rad/フレーム・
   符号反転17.5回/秒・29%のフレームが急転回）。
   → 回り込み中は**分離を外力チャネル（kick）へ回して位置で押し分ける**
   （`ORBIT_SEP_KICK_ACCEL = 300`）。向きは接線だけを見る。
   実測：12/40/80体すべてで**符号反転0回・急転回0%**、回転量は理想値 v/R に一致。

群れが増えると輪が詰まって重なるので、**輪の半径は群れの数で広がる**
（`ORBIT_RING_ROWS = 3` 列ぶんまで詰め込む前提。80体で半径119px）。

### 12-2e. 弧長トレース（`pathTrace`）— v0.37.1

**「旋回に制約があるように見える。線をそのままなぞってほしい」**（UFO への実機判定・2026-08-14）への対応。
旋回上限を外しても線をなぞらないのは、**操舵の仕組みそのものが角を丸める**から：

1. リーダーが「数点先のウェイポイント」を目標に**操舵**するので、角の内側を突っ切る。
2. 竜はリーダーの**現在位置**を追うので、リーダーが角を曲がりきった後は竜が斜めに近道する。

そこで `pathTrace` の竜種は操舵をやめる。

| | 通常の竜 | `pathTrace` の竜 |
|---|---|---|
| リーダー | 数点先へ**操舵**（速度を指数平滑） | 線の上を**弧長で進む**（位置を直接置く。線から一切外れない） |
| 竜が追う点 | リーダーの現在位置 | **線上の隊列スロットの少し先**（`PATH_TRACE_LOOKAHEAD = 36px`） |
| 閉じた線 | 折り返しなしで周回 | 弧長を全長で剰余＝**描いた円をそのまま周回** |

- 隊列は「1行 N 機 × 行間32px」。**列数は群れの数で決まる**（`round(sqrt(生存数/3))` を 3〜7 に丸める）。
  少数のときほど細くして線に重ね、増えたら横に広がる（1列で伸ばすと100体で1000px超の蛇になる）。
- スロットを直接狙わず **LOOKAHEAD ぶん先**を狙うのが要点。スロットに乗った瞬間は
  目標までの距離が0になり狙う向きが定まらないので、等速で止まれない竜はその場で震える。
- 行間・列間は分離半径（28）より広く取る。狭いと隊列を組んだ瞬間に押し合って線から弾かれる。
- 弧が伸びないまま `PATH_TRACE_IDLE_HOLD = 0.3秒` 経ったら（指を止めた／末端に着いた）
  隊列を解いて操縦点追従へ戻す＝そこからは回り込み・末端旋回に自然につながる。
- 線から遠い所で描き始めたときは、`PATH_TRACE_SNAP_DIST = 70px` まで寄るまで弧を進めない
  （＝**線の頭からなぞる**）。

**実測**（ジグザグ＝鋭角の連続を描いて20体で追従。「ズレ」は描いた線からの距離）：

| 竜種 | 群れ重心のズレ（平均/最大） | 各竜のズレ（平均/最大） |
|---|---|---|
| **UFO（pathTrace）** | **11.8 / 40.0** | **38.5 / 67.5** |
| 火竜（通常の操舵） | 152.6 / 518.2 | 159.9 / 519.1 |

各竜のズレ38.5は**隊列の幅そのもの**（3列＝±32px）が大半で、追従誤差は10px前後。
リーダーは線の上に完全に乗る（ズレ0.0）。閉じた円では半径198〜200pxを保って周回した（描いた円は半径200）。

**副作用**：`leaderSpeed` を `maxSpeed` と同値（175）にした。スロットが竜より速く逃げると
隊列が保てないため。指を175px/sより速く動かすとリーダーが線の上を追いきれず群れが遅れる
（＝線はなぞるが指には追いつかない）。ここは実機の手触りで詰める。
...
### 12-2f. 矢印操作（`controlScheme: "vector"`）— v0.39

**「動きは面白いが操作感が悪くなりすぎる」**（爆竜への実機判定・2026-08-18）への対応。
旋回半径179・加速90・止まれない群れに**線をなぞらせる**のが破綻の原因だった。
曲がれない機体に曲線を指示できてしまうので、指示と結果が永久に一致しない。

そこで**動きは一切変えず、線の解釈と表示だけ**を差し替えた。

| | "path"（従来） | "vector"（爆竜） |
|---|---|---|
| なぞっている間 | 指が常に操縦点＝リアルタイムに操縦 | **操縦しない**（群れは今の進路のまま飛ぶ）。矢印で予告だけ出す |
| 指を離した瞬間 | パス末端の挙動（`pathEndMode`）へ | **進路が確定**。以後リーダーはその向きへ進み続ける |
| 線の形 | 軌跡として使う | **始点→終点のベクトルだけ**使う（途中の形は捨てる） |
| タップ（なぞりが `VECTOR_MIN_STROKE = 26px` 未満） | ほぼ無視 | **群れの重心→触れた点**を進路にする |

**物理は共通のまま**：リーダーも群れも既存のものをそのまま使う。差分は
`updateLeaderVector`（リーダーの目標の決め方）と `drawVectorAim`（表示）だけ。

- 表示は群れの重心に出す。照準中＝明るい太矢印（`VECTOR_AIM_LEN = 150px`）、
  確定後＝細い矢印（`VECTOR_BEARING_LEN = 96px`）。確定の瞬間だけ `VECTOR_COMMIT_FX = 0.45秒` 強く光る。
- **確定後の矢印は `VECTOR_BEARING_SHOW = 2.2秒` で消える**（最後の `VECTOR_BEARING_FADE = 0.7秒` でフェード）。
  v0.39 では出しっぱなしにしていたが、進路は群れの向きそのものが語るので画面に常駐させる必要がない
  （実機判定・2026-08-18）。新しくなぞれば必ずまた出るし、**縁で進路が勝手に曲げられたときも出し直す**
  （0.05rad 以上変わったときだけ。縁を沿って走っている間ずっと出っぱなしにはしない）。
- **世界の縁では進路を壁沿いへ確定し直す**（`deflectVectorBearingAtWall`）。
  成分を殺すだけだと縁を出た瞬間に元の進路へ戻って往復し、**リーダーの速さが0を通る**
  （実測 minSpeed 0＝群れが縁でガタつく）。進路そのものを書き換えれば安定し、矢印表示とも一致する。
  さらに真横ではなく `VECTOR_WALL_ESCAPE = 0.3` だけ内向きに混ぜて、縁への貼り付きを防ぐ。

**実測**（群れ20体）：45度になぞって離す→進路45.0度で確定、群れの実移動45.3度・10秒で1301px。
タップは重心→点の角度と0.1度以内で一致。なぞっている最中は進路が変わらない（＝操縦されない）。
左壁へ真正面から突っ込ませても最小速度76px/s（停止しない）、竜が世界の縁（10px以内）に
触れたのは18,000サンプル中27フレームだけ。

**この竜種が失うもの**：線が軌跡ではなくなるので、**「円を描いて放置＝周回」がなくなる**。
ただし実機判定は逆で、**「まっすぐ進み続けて爆撃し続けるので、むしろ爆竜が一番アクアリウムを実現している」**
（2026-08-18）。周回だけがアクアリウムではなく、**放置しても勝手に世界が壊れ続ける**ことが本質だった。

**旧操作は別の竜種として残してある**（惑竜 / `wayward`）。動きも攻撃も爆竜と完全に同一で、
`controlScheme` だけが既定の "path"。**操作方式そのものを比較するためのペア**として機能する
（「思い通りに曲がらないまごつき」も面白い、という判定だったため）。
...
| **hover**（プリセット） | INSTANT | 170 | — | 即時（1フレーム） | 0.02 | 33 | 4.4px |
| **ground**（プリセット） | 1.0 | 105 | 105 | 1.0 | 0.73 | 162〜206 | 1.9px |

**v0.37 の極端枠3種**（群れ20体・指を300px振幅の8の字で振り回したときの実測。
「散らばり」は群れ重心からの平均距離、「操縦点との差」は操縦点からの平均距離）：

| 竜種 | turnRate | maxSpeed | 旋回半径 | 散らばり（平均/最大） | 操縦点との差 |
|---|---|---|---|---|---|
| 火竜（基準） | 2.0 | 160 | 80 | 42 / 49 | 252 |
| ガーゴイル（既存で最も鈍い） | 1.4 | 150 | 107 | 126 / 186 | 321 |
| **UFO** | INSTANT | 175（等速） | — | **48 / 61** | **210**（最も追従が良い） |
| **突竜** | 0.95 | 170 | 179 | **140 / 208** | **353**（最も置いていかれる） |
| **蛙竜** | 5.0（着地時） | 150（平均） | — | 78 / 94 | 257 |

- 突竜は `turnRate 0.8`（半径219）だと平均258px離れ、カメラ倍率1.05の実効画面371pxから
  溢れた。**0.95 は「画面に収まる範囲での最悪」**。もっと極端にしたいならここを下げる。
...
| `"orbit"`（v0.37） | **末端の点を中心に旋回し続ける** | 半径95pxで連続周回（5秒で1.7周）。中心はパス末尾の点そのものなので状態を持たない |

- `"orbit"` の実半径は `orbitRadius` より**大きく出る**（UFO は指定64に対し実測95）。
  リーダーの操舵が指数平滑（`k = dt*4`）なので、`leaderSpeed 215` ではそこまでしか曲がれない。
  半径を詰めたいならこのゲインを上げる（上げすぎると行き過ぎて振動する）。
...
`MOVE_PROFILE_PRESETS` に `hover` / `ground` を定義済み。**竜種には割り当てていない**ので、
実機の手触り確認は**デバッグパネルの「🛸 動き切替」**で行う
（左下バージョンを3タップ → パネル。`種:xxx` → `hover` → `ground` を巡回。現在値はステータス行に出る）。

- `hover`: `accel`/`turnRate` ともに `INSTANT`、`minSpeed: 0`、`pathEndMode: "stop"`。
  指に対して向きと速さが即時に変わり、**止まれる**。
  ※v0.36 まで `ufo` という名前だったが、v0.37 で竜種 `ufo`（等速で止まれない・回り込む）が
  実装されて紛らわしくなったので改名した。両者は別物。
