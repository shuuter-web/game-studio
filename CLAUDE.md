...
2. **発散**（idea-scout）: **選びたい数の10倍**を目安に出す（3案選ぶなら30案）。母数が足りないと
   選別の手間のほうが大きくなる。**出た数が20件程度までなら、絞らずに全部そのまま見せる**
   （こちらで精査・選別してから渡さない。選ぶのはユーザー）。
   **1件につき ログライン／コアループ／成長と爽快の軸／失敗条件 の4点は必ず埋める**。
   一行の見出しだけでは遊びとして成立するか判断できず、数を出す意味が消える
   （失敗条件が無いものは遊びではなく玩具）。
   **設定・フレーバー（誰が・どこで・なぜ）を先に立てる**。遊び方と動きの軸だけの案は薄い。
   既存の試作を土台にする回は、**「土台のどこをどう使うか」を1行で必ず書く**
   （書けない案は、その土台にする意味がない）。
...
Seed Sprint の結果を企画書として公開するときは、**各案の全文も必ず一緒に公開してリンクする**
（要約だけ読める状態にしない。判断の根拠が追えなくなるため）。

1. `node tools/build-seed-pages.mjs` … `ideas/seeds/*.md` から `docs/seeds/*.html` を生成する
   （種の全文ページ・一覧・発散ログ）。**生成物もコミットする**。`--check` で最新かだけ確認できる
2. 企画書 `docs/pitch/<名前>/index.html` を書く。各案のカードから `../../seeds/PS-XXX-*.html` へリンクし、
   フッターからも一覧 `../../seeds/` と発散ログ `../../seeds/log.html` へ張る
3. ルート `index.html` のポータルにカードを追加する
4. headless で検証（表示崩れ・リンク切れ・light/dark・モバイル幅）してから master へ反映する
5. 前の企画書に**実測などで覆った前提がある場合は、消さずに訂正の追記を入れて**新しい企画書へ導線を張る
